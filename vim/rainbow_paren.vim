...
if exists("g:rainbow") && g:rainbow != 0
  hi level1c ctermfg=lightred guifg=lightred       " this color is never nested, it only appears on the outermost layer
  hi level2c ctermfg=yellow guifg=yellow 
  hi level3c ctermfg=lightblue guifg=lightblue 
  hi level4c ctermfg=lightgreen guifg=lightgreen 
  hi level5c ctermfg=lightred guifg=lightred       " this color is never nested, it only appears on the outermost layer
  ...
  call s:DoSyntaxes(5) " 6 is the number of colors we have
